...
class Black_Hole(Simulton):  
    ...
    def update(self, model):
        ...
        remove_set = {sim for sim in model.find(if_prey) if self.contains((sim.get_location()))}
        for sim in remove_set:
            model.remove(sim)
        return remove_set
    ...
    def display(self, canvas):
        ...
        canvas.create_oval(x-self.radius, y-self.radius, x+self.radius, y+self.radius, fill = 'black') 
